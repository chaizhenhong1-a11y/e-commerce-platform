...
async function pathOf(ctx:{params:Promise<{imageId:string}>}){const {imageId}=await ctx.params;return `/staff/catalog/images/${encodeURIComponent(imageId)}`;}
export async function PATCH(request:NextRequest,ctx:{params:Promise<{imageId:string}>}){return proxy(await pathOf(ctx),"PATCH",request);}
export async function DELETE(request:NextRequest,ctx:{params:Promise<{imageId:string}>}){return proxy(await pathOf(ctx),"DELETE",request);}

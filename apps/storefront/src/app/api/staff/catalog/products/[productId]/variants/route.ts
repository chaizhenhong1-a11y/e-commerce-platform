...
export async function POST(request:NextRequest,{params}:{params:Promise<{productId:string}>}){const {productId}=await params;return proxy(`/staff/catalog/products/${encodeURIComponent(productId)}/variants`,"POST",request);}

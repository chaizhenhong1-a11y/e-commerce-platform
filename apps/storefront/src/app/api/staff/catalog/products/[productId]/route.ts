...
async function pathOf(ctx:{params:Promise<{productId:string}>}){const {productId}=await ctx.params;return `/staff/catalog/products/${encodeURIComponent(productId)}`;}
export async function GET(_request:NextRequest,ctx:{params:Promise<{productId:string}>}){return proxy(await pathOf(ctx),"GET");}
export async function PUT(request:NextRequest,ctx:{params:Promise<{productId:string}>}){return proxy(await pathOf(ctx),"PUT",request);}
export async function PATCH(request:NextRequest,ctx:{params:Promise<{productId:string}>}){return proxy(await pathOf(ctx),"PATCH",request);}

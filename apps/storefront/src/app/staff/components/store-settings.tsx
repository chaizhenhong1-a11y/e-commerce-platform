...
export function StoreSettingsManager() {
  ...
  const [initialForm, setInitialForm] = useState<UpdateStoreSettingsInput | null>(null);
  ...
  const moneyInput = (
    label: string,
    key: "standardShippingCents" | "freeShippingThresholdCents",
    hint: string,
  ) => (
    <label className={styles.field}>
      <span>{label}</span>
      <input
        ...
        inputMode="decimal"
        required
        value={(Number(form[key] || 0) / 100).toFixed(2)}
        onChange={(event) => {
          const ringgit = Number(event.target.value);
          set(key, Number.isFinite(ringgit) ? Math.round(ringgit * 100) : 0);
        }}
      />
        ...
        {!loading ? (
          ...
          <section id="settings-delivery" className={styles.section}>
            <div className={styles.sectionHeading}>
              ...
              <p>Set customer-facing delivery charges in normal currency amounts. The system handles cents internally.</p>
            </div>
            <div className={styles.grid}>
              {moneyInput(
                `Standard shipping fee (${form.currency || "MYR"})`,
                "standardShippingCents",
                "Enter the amount customers pay for standard delivery, for example 8.00.",
              )}
              {moneyInput(
                `Free shipping from (${form.currency || "MYR"})`,
                "freeShippingThresholdCents",
                Number(form.freeShippingThresholdCents || 0) > 0
                  ? `Orders from ${form.currency || "MYR"} ${(Number(form.freeShippingThresholdCents) / 100).toFixed(2)} qualify for free delivery.`
                  : "Enter 0.00 if there is no free-delivery threshold.",
              )}

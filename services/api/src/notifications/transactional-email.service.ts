...
export class TransactionalEmailService {
  ...
  async deliver(input: {
    ...
    try {
      ...
      const text = `${greeting}\n\n${input.message}\n\nView in Elvane: ${actionUrl}`;
      const html = `<!doctype html><html><body style="margin:0;background:#f6f7fb;font-family:Arial,sans-serif;color:#171717"><div style="max-width:600px;margin:0 auto;padding:32px 20px"><div style="background:#fff;border:1px solid #e7e7ee;border-radius:16px;padding:28px"><div style="font-size:22px;font-weight:700;margin-bottom:22px">Elvane</div><p>${this.escape(greeting)}</p><p style="line-height:1.6">${this.escape(input.message)}</p><p style="margin-top:28px"><a href="${this.escape(actionUrl)}" style="display:inline-block;background:#111827;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px">View update</a></p><p style="margin-top:28px;color:#6b7280;font-size:13px">This is a transactional message about your Elvane account or order.</p></div></div></body></html>`;
